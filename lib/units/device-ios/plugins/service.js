...
module.exports = syrup.serial()
  ...
  .define(function(options, wda, router, push,storage) {
    ...
    const typeQueue = (function (wda) {
      ...
      setInterval(function() {
        if (lastRequestHasCompleted && textQueue.length>0) {
          ...
          wda.PostData('wda/keys',{value: textQueue.splice(0, textQueue.length)},true,1).then(function() {
    ...
    plugin.goHome = function(){
        ...
        wda.PostData('wda/pressButton',{name:'home'},true)
    }
    ...
    function wheelCommit() {
      ...
      wda.PostData( 'wda/wheel', {dist:toWheel}, true, function( val ) {
        wheelRunning = 0;
      } ).then( function() {
        if( again ) wheelCommit();
      } );
    }
    ...
    plugin.doWheel = function( dist ) {
      ...
      if( lastWheel ) {
        ...
        var timeDif = curTime - lastWheel;
        if( timeDif < 500 ) {
          lastWheel = curTime;
          clearTimeout( wheelCommitTimer );
          console.log("Curval",wheelSum, "Adding", dist*1);
          wheelSum = wheelSum + dist*1;
          wheelCommitTimer = setTimeout( wheelCommit, 500 );
        }
        return;
      }
      wheelSum = dist*1;
